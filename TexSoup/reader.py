...
ELEMENTARY_PARTICLES = {'def', 'item', 'let', 'input'}
TEX_PRIMERTIES = {'epsfxsize', 'unitlength'}
EXTRAS = ELEMENTARY_PARTICLES | TEX_PRIMERTIES
...
def read_tex(src):
    ...
    if c.startswith('%'):
        ...
        return read_math_env(src, TexEnv(name, [], nobegin=True))
    elif c.startswith((r'\[', r'\(')):
        # Fisnik
        name = "displaymath" if c.startswith(r'\[') else "math"
        begin = r"\[" if c.startswith(r'\[') else r"\("
        end = r"\]" if c.startswith(r'\[') else r"\)"
        return read_math_env(src, TexEnv(name, [], nobegin=True, begin=begin, end=end))
    elif c.startswith('\\'):
        command = TokenWithPosition(c[1:], src.position)
        mode, expr = None, None # Fisnik 
        if command == 'begin': # Fisnik 
            mode, expr, _ = 'begin', TexEnv(src.peek(1)), src.forward(3)
            expr.args = read_args(src, expr.args)
            read_env(src, expr)
            return expr
        elif command in EXTRAS: # Fisnik 
...
def read_item(src):
    ...
    while (src.hasNext() and not str(src).strip(" ").startswith(('\n', '\n\n')) and
            not src.startswith(('\\item', '\\end')) and

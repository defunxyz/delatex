...
class TexNode(object):
    ...
    def replace(self, child, *nodes):
        ...
        """
        # Fisnik 
        if child.expr in self.expr._contents: 
            self.expr.insert(
                self.expr.remove(child.expr),
                *nodes)
        else: 
            print(f'TeX child expression: "{child.expr}" does not exist in "_contents" attribute.')
...
class TexExpr(object):
    ...
    def _supports_contents(self):
        # Fisnik 
        return True if (getattr(self, '_contents', None) is not None) else False 

    def _assert_supports_contents(self):
        # Fisnik 
        # if self._supports_contents(): 
        #     raise TypeError(f'Node {self.name} does not support _contents attribute')
        pass 
        ...
        if isinstance(s, (list, tuple)):
            for arg in arg_type:
                if [s[0], s[-1]] == arg.delims():
                    return arg(*s[1:-1])
            raise TypeError("""Malformed argument. First and last elements must match a valid argument format. Common issues include:\nUnescaped special characters, mistyped closing punctuation, misalignment.""")
        for arg in arg_type:
            if arg.__is__(s):
